import React, {forwardRef} from 'react'

const NavBarNotifications = forwardRef(function NavBarNotifications({showNotifications, notifications = []}, ref) {
    return (
        <div ref={ref}>
            {showNotifications && <div
                className="tw-absolute tw-bg-white tw-shadow-md tw-h-[80vh] tw-w-[90vw] tw-max-w-[400px] tw-right-0 -tw-bottom-1 tw-translate-y-[100%] tw-flex tw-flex-col tw-overflow-hidden">
                <div className="tw-relative tw-flex tw-flex-col tw-overflow-y-auto ">
                    <div className="tw-p-6 tw-flex tw-flex-row tw-w-full tw-justify-between tw-items-center">
                        <h1 className="tw-text-cyan-900 tw-font-semibold tw-text-xl ">
                            Notificaciones
                        </h1>
                        <button onClick={() => {
                            document.getElementById('toggleNotifications').click()
                        }}
                                className="tw-font-semibold tw-text-cyan-900 tw-border-none tw-outline-none tw-bg-transparent">
                            Cerrar
                        </button>
                    </div>

                    <div
                        className="tw-flex tw-flex-1 tw-flex-col tw-flex-grow tw-custom-scroll tw-overflow-y-auto tw-p-4 tw-gap-4">
                        {notifications?.length >= 1 ? notifications.map(({
                                                                            audience,
                                                                            description,
                                                                            generateOn,
                                                                            id,
                                                                            title
                                                                        }, index) => (
                            <div
                                key={index}
                                className="tw-flex tw-flex-row tw-bg-white tw-shadow-small tw-p-3 tw-gap-2 tw-rounded-lg"
                            >
                                <img
                                    src="data:image/jpeg;base64,"
                                    alt="icon"
                                    className="tw-rounded-full tw-object-cover tw-h-10 tw-w-10"
                                />
                                <div className="tw-flex tw-flex-col">
                                    <div className="tw-flex tw-flex-row tw-items-center tw-justify-between">
                                        <div className="tw-text-sm tw-font-semibold">
                                            {title}
                                        </div>
                                        <div className="tw-flex tw-flex-row tw-items-center tw-gap-3 tw-text-sm">
                                            <div
                                                className="tw-text-gray-600">{new Date(generateOn).toDateString()}</div>
                                            <div className="tw-py-px tw-px-2 tw-rounded-md tw-text-white tw-bg-sky-600">
                                                Audience: {audience}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="tw-text-sm tw-text-gray-500 tw-pt-2">
                                        {description}
                                    </div>
                                </div>
                            </div>
                        )) : <div
                            className="tw-flex tw-flex-row tw-bg-white tw-shadow-small tw-p-3 tw-gap-2 tw-rounded-lg"
                        >
                            <div className="tw-flex tw-flex-col">
                                <div className="tw-flex tw-flex-row tw-items-center tw-justify-between">
                                    <div className="tw-text-sm tw-font-semibold">
                                        No hay notificaciones.
                                    </div>
                                </div>
                            </div>
                        </div>}
                    </div>
                </div>
            </div>}
        </div>

    )
})

export default NavBarNotifications
